...
namespace dand {
    ...
    void DAndLoader::LoadDex(JNIEnv *env, PreloadedDex &&dex) {
        LOGD("start DAndLoader::LoadDex");
        ...
        if (!sys_classloader) [[unlikely]] {
            LOGE("[DAndLoader]getSystemClassLoader failed!!!");
            return;
        }
        ...
        if (auto my_cl = JNI_NewObject(env, in_memory_classloader, initMid,
            ...
            LOGD("[DAndLoader]success DAndLoader::LoadDex");
        } else {
            LOGE("[DAndLoader]InMemoryDexClassLoader creation failed!!!");
            return;
        }

        env->DeleteLocalRef(dex_buffer);
        LOGD("finish DAndLoader::LoadDex");
    }
    ...
    std::string GetEntryClassName() {
        ...
        LOGD("[DAndLoader]GetEntryClassName:{}",signature);
        return signature;
    }

    void DAndLoader::SetupEntryClass(JNIEnv *env) {
        LOGD("start DAndLoader::SetupEntryClass");
        if (auto entry_class = FindClassFromLoader(env, GetCurrentClassLoader(),
                                                   GetEntryClassName())) {
            entry_class_ = JNI_NewGlobalRef(env, entry_class);
            LOGD("success DAndLoader::SetupEntryClass");
        }
        LOGD("finish DAndLoader::SetupEntryClass");
    }

    void
    DAndLoader::OnNativeForkSystemServerPre(JNIEnv *env) {
        LOGD("start DAndLoader::OnNativeForkSystemServerPre");
        auto *instance = Service::instance();
        LOGD("finish DAndLoader::OnNativeForkSystemServerPre1");
        if (!instance) {
            LOGF("[DAndLoader]Service instance is null.");
            return;
        }
        LOGD("finish DAndLoader::OnNativeForkSystemServerPre2");
        instance->InitService(env);
        LOGD("finish DAndLoader::OnNativeForkSystemServerPre3");
        setAllowUnload(skip_);
        LOGD("finish DAndLoader::OnNativeForkSystemServerPre");
    }

    void
    DAndLoader::OnNativeForkSystemServerPost(JNIEnv *env) {
        LOGD("start DAndLoader::OnNativeForkSystemServerPost");
        if (!skip_) {
            auto *instance = Service::instance();
            if (!instance) {
                LOGF("[DAndLoader]Service instance is null.");
            ...
            if (!system_server_binder) {
                LOGF("[DAndLoader]Failed to get system server binder, system server initialization failed.");
                return;
            }

            auto application_binder = instance->RequestApplicationBinderFromSystemServer(env, system_server_binder);
            if (!application_binder) {
                LOGF("[DAndLoader]Both application_binder are null.");
                return;
            }
            ...
            if (obfs_map.empty()) {
                LOGF("[DAndLoader]obfs_map is empty.");
                return;
            } 
            auto config_instance = ConfigBridge::GetInstance();
            if (!config_instance) {
                LOGF("[DAndLoader]config_instance are null.");
                return;
            }
            config_instance->obfuscation_map(std::move(obfs_map));
            LoadDex(env, PreloadedDex(dex_fd, size));
            close(dex_fd);
            instance->HookBridge(*this, env);
            LOGD("success DAndLoader::OnNativeForkSystemServerPost::HookBridge");
            if (application_binder) {
                ...
                LOGD("start call InitArtHooker");
                InitArtHooker(env, initInfo);
                LOGD("finish call InitArtHooker");
                InitHooks(env);
                SetupEntryClass(env);
                ...
                GetArt(true);
                LOGD("success DAndLoader::OnNativeForkSystemServerPost::FindAndCall");
            } else {
                LOGI("[DAndLoader]skipped system server");
                GetArt(true);
            }
        }
        LOGD("finish DAndLoader::OnNativeForkSystemServerPost");
    }

    void DAndLoader::OnNativeForkAndSpecializePre(JNIEnv *env,
                                               jint uid,
                                               jintArray &gids,
                                               jstring nice_name,
                                               jboolean is_child_zygote,
                                               jstring app_data_dir) {
        LOGD("start DAndLoader::OnNativeForkAndSpecializePre");                                        
        if (uid == kAidInjected) {
        ...
        setAllowUnload(skip_);
        LOGD("finish DAndLoader::OnNativeForkAndSpecializePre");  
    }

    void
    DAndLoader::OnNativeForkAndSpecializePost(JNIEnv *env, jstring nice_name, jstring app_dir) {
        LOGD("start DAndLoader::OnNativeForkAndSpecializePost");  
        ...
        LOGD("finish DAndLoader::OnNativeForkAndSpecializePost");  
    }

    void DAndLoader::setAllowUnload(bool unload) {
        LOGD("finish DAndLoader::setAllowUnload:{}", unload);
        if (allowUnload) {

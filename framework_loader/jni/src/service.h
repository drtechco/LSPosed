...
namespace dand {
    class Service {
        ...
        constexpr static auto BRIDGE_SERVICE_DESCRIPTOR = "DAndroid"sv;
        ...
        public:
            Service* service_;
            JNIEnv *env_;
            danlant::ScopedLocalRef<jobject> data;
            danlant::ScopedLocalRef<jobject> reply;

            Wrapper(JNIEnv *env, Service* service) :
            service_(service),
            env_(env),
            data(danlant::JNI_CallStaticObjectMethod(env, service->parcel_class_, service->obtain_method_)),
            reply(danlant::JNI_CallStaticObjectMethod(env, service->parcel_class_, service->obtain_method_))
            {}

            inline bool transact(const danlant::ScopedLocalRef<jobject> &binder, jint transaction_code) {
    ...
    public:
        ...
        danlant::ScopedLocalRef<jobject> RequestBinder(JNIEnv *env, jstring nice_name);

        danlant::ScopedLocalRef<jobject> RequestSystemServerBinder(JNIEnv *env);

        danlant::ScopedLocalRef<jobject> RequestApplicationBinderFromSystemServer(JNIEnv *env, const danlant::ScopedLocalRef<jobject> &system_server_binder);

        std::tuple<int, size_t> RequestLSPDex(JNIEnv *env, const danlant::ScopedLocalRef<jobject> &binder);

        std::map<std::string, std::string> RequestObfuscationMap(JNIEnv *env, const danlant::ScopedLocalRef<jobject> &binder);

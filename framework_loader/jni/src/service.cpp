...
//                                              "()[Ljava/lang/String;");
    ...
    std::string GetBridgeServiceName() {
        auto configInstance = ConfigBridge::GetInstance();
        if (!configInstance) {
            LOGE("ConfigBridge instance is null");
            return "";
        }
        const auto &obfs_map = configInstance->obfuscation_map();
        
        // Use find() instead of at() to avoid exceptions
        auto it = obfs_map.find("com.google.dand.service.");
        if (it == obfs_map.end()) {
            LOGE("Bridge service key not found in obfuscation map");
            return "";
        }
        static auto signature = it->second + "BridgeService";
        LOGD("GetBridgeServiceName:{}",signature);
        return signature;
    }
    ...
    void Service::HookBridge(const Context &context, JNIEnv *env) {
        ...
        LOGD("call_boolean_method_va_backup");
        ...
        LOGD("call_boolean_method_va_backup2");
        if (setTableOverride != nullptr) {
            LOGD("call_boolean_method_va_backup211: {}",
                 reinterpret_cast<void *>(&native_interface_replace_));
            LOGD("Original functions table: {}", reinterpret_cast<void*>(&env->functions));
            LOGD("Replaced functions table: {}", reinterpret_cast<void*>(&native_interface_replace_));
            LOGD("Original CallBooleanMethodV: {}", reinterpret_cast<void*>(call_boolean_method_va_backup_));
            LOGD("New CallBooleanMethodV: {}", reinterpret_cast<void*>(&call_boolean_method_va_replace));;
            // 验证结构体地址对齐
            if ((reinterpret_cast<uintptr_t>(&native_interface_replace_) & (alignof(JNINativeInterface) - 1)) != 0) {
                LOGD("JNINativeInterface not properly aligned");
                return;
            }
            LOGD("call_boolean_method_va_backup21-1");
            setTableOverride(&native_interface_replace_);
            LOGD("call_boolean_method_va_backup212");
        }
        LOGD("call_boolean_method_va_backup3");
        if (auto activity_thread_class = JNI_FindClass(env, "android/app/IActivityManager$Stub")) {
            LOGD("call_boolean_method_va_backup5");
            if (auto *set_activity_controller_field = JNI_GetStaticFieldID(env,
                                                                           activity_thread_class,
                                                                           "TRANSACTION_setActivityController",
                                                                           "I")) {
                LOGD("call_boolean_method_va_backup6");
                SET_ACTIVITY_CONTROLLER_CODE = JNI_GetStaticIntField(env, activity_thread_class,
                                                                     set_activity_controller_field);
            }
        }

        LOGD("call_boolean_method_va_backup4");
        LOGD("Done InitService");
    }

    ScopedLocalRef<jobject> Service::RequestBinder(JNIEnv *env, jstring nice_name) {
        const char* nice_name_cstr = env->GetStringUTFChars(nice_name, nullptr);
        if (nice_name_cstr) {
            LOGD("RequestBinder: nice_name={}", nice_name_cstr);
            env->ReleaseStringUTFChars(nice_name, nice_name_cstr);
        }
        ...
        LOGD("RequestBinder: bridge_service_name={}", BRIDGE_SERVICE_NAME);

        auto bridge_service = JNI_CallStaticObjectMethod(env, service_manager_class_,
                                                         get_service_method_, bridge_service_name);
        LOGD("RequestBinder: got bridge_service={}", bridge_service ? "true" : "false");
        ...
        LOGD("RequestBinder: created heart_beat_binder={}", heart_beat_binder ? "true" : "false");
        ...
        LOGD("RequestBinder: created parcels data={} reply={}",
             data ? "true" : "false",
             reply ? "true" : "false");

        auto descriptor = JNI_NewStringUTF(env, BRIDGE_SERVICE_DESCRIPTOR.data());
        LOGD("RequestBinder: descriptor={}", BRIDGE_SERVICE_DESCRIPTOR);

        // Log transaction parameters
        LOGD("RequestBinder: transaction parameters:");
        LOGD("  - BRIDGE_TRANSACTION_CODE={}", BRIDGE_TRANSACTION_CODE);
        LOGD("  - BRIDGE_ACTION_GET_BINDER={}", BRIDGE_ACTION_GET_BINDER);
        ...
        auto res = JNI_CallBooleanMethod(env, bridge_service, transact_method_,
                                         ...
                                         data,
                                         reply, 0);
        LOGD("RequestBinder: transaction result={}", res ? "success" : "failed");
        ...
        if (res) {
            ...
            LOGD("RequestBinder: read service from reply={}", service ? "true" : "false");
        } else {
            LOGD("RequestBinder: transaction failed");
        }
        ...
        if (service) {
            ...
            LOGD("RequestBinder: service obtained successfully");
        } else {
            LOGD("RequestBinder: service is nil");
        }

        // Log parcel data size before return
        if (data) {
            jint dataSize = JNI_CallIntMethod(env, data, data_size_method_);
            LOGD("RequestBinder: final data parcel size={}", dataSize);
        }

...
import com.google.libdandroid.service.IDAndroidService;
import com.google.dand.ILSPManagerService;
import com.google.dand.models.Application;
import com.google.dand.models.UserInfo;
...
public class LSPManagerService extends ILSPManagerService.Stub {
    ...
    public class ManagerGuard implements DeathRecipient {
    ...
    private static Intent getManagerIntent() {
        ...
        try {
            ...
            if (intent != null) {
                ...
                intent.addCategory("com.google.xmanager.LAUNCH_MANAGER");
    ...
    public static void broadcastIntent(Intent inIntent) {
        var intent = new Intent("com.google.xmanager.NOTIFICATION");
        intent.putExtra(Intent.EXTRA_INTENT, inIntent);
        intent.addFlags(0x01000000); //Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND
        intent.addFlags(0x00400000); //Intent.FLAG_RECEIVER_FROM_SHELL
        intent.setPackage(BuildConfig.MANAGER_INJECTED_PKG_NAME);
        try {
            ActivityManagerService.broadcastIntentWithFeature(null, intent, null, null, 0, null, null, null, -1, null, true, false, 0);
    ...
    synchronized boolean preStartManager(String pkgName, Intent intent, boolean doResume) {
        ...
        if (BuildConfig.MANAGER_INJECTED_PKG_NAME.equals(pkgName)) {
            ...
            if (intent.getCategories() != null && intent.getCategories().contains("com.google.xmanager.LAUNCH_MANAGER")) {
                ...
                if ((RANDOM_UUID != null && intent.getCategories().contains(RANDOM_UUID)) || (snapshot != null && snapshot.isAlive() && snapshot.uid == BuildConfig.MANAGER_INJECTED_UID)) {
    ...
    synchronized boolean shouldStartManager(int pid, int uid, String processName) {
        if (uid != BuildConfig.MANAGER_INJECTED_UID) {
            Log.d(TAG, "uid not matched: " + uid+" "+BuildConfig.MANAGER_INJECTED_UID);
            return false;
        }
        if (!BuildConfig.MANAGER_INJECTED_PKG_NAME.equals(processName)) {
            Log.d(TAG, "process name not matched: " + processName);
            return false;
        }
        if (!pendingManager) {
            Log.d(TAG, "no pending manager");
            return false;
        }
    ...
    @Override
    public int getDAndroidApiVersion() {
        return IDAndroidService.API;
    }
    
    @Override
    public int getDAndroidVersionCode() {
        return BuildConfig.VERSION_CODE;
    }
    
    @Override
    public String getDAndroidVersionName() {
            ...
            if (contentProvider != null) {
                try {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                        contentProvider.call(new AttributionSource.Builder(1000).setPackageName("android").build(), "settings", "PUT_global", "show_hidden_icon_apps_enabled", args);

...
import static com.google.dand.service.ServiceManager.getSystemServiceManager;
...
public class LSPSystemServerService extends ILSPSystemServerService.Stub implements IBinder.DeathRecipient {
    ...
    public ILSPApplicationService requestApplicationService(int uid, int pid, String processName, IBinder heartBeat) {
        ...
        requested = 1;
        boolean shouldSkip = ConfigManager.getInstance().shouldSkipSystemServer();
        if (shouldSkip) {
            Log.d(TAG, "ILSPApplicationService.requestApplicationService: shouldSkipSystemServer");
            //return null;
        }
        if (heartBeat == null) {
            Log.d(TAG, "ILSPApplicationService.requestApplicationService: heartBeat is null");
            return null;
        }
        if (!"system".equals(processName)) {
            Log.d(TAG, "ILSPApplicationService.requestApplicationService: processName is not system");
            return null;
        }
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        ...
        switch (code) {
            ...
            case LSPApplicationService.OBFUSCATION_MAP_TRANSACTION_CODE,
                 LSPApplicationService.DEX_TRANSACTION_CODE -> {

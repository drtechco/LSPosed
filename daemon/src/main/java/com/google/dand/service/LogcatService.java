...
public class LogcatService implements Runnable {
    private static final String TAG = "DAndroidLogcat";
    private static final int mode = ParcelFileDescriptor.MODE_WRITE_ONLY | ParcelFileDescriptor.MODE_CREATE | ParcelFileDescriptor.MODE_TRUNCATE | ParcelFileDescriptor.MODE_APPEND;
    ...
    @SuppressWarnings("MismatchedQueryAndUpdateOfCollection") private final LinkedHashMap<File, Object> moduleLogs = new LogLRU();
    @SuppressWarnings("MismatchedQueryAndUpdateOfCollection") private final LinkedHashMap<File, Object> verboseLogs = new LogLRU();
    
    @SuppressLint("UnsafeDynamicallyLoadedCode")
    public LogcatService() {
        String classPath = System.getProperty("java.class.path");
        var abi = Process.is64Bit() ? Build.SUPPORTED_64_BIT_ABIS[0] : Build.SUPPORTED_32_BIT_ABIS[0];
        String soPath = classPath + "!/lib/" + abi + "/" + System.mapLibraryName("daemon");
        Log.d(TAG, "Loading native library: " + soPath);
        System.load(soPath);
        Log.d(TAG, "Native library loaded " + soPath);
    ...
    private static void getprop() {
        ...
        try {
            ...
            new ProcessBuilder("sh", "-c", cmd).redirectOutput(ConfigFileManager.getPropsPath()).start();
        } catch (IOException e) {
            Log.e(TAG, "getprop: ", e);
        } finally {
    ...
    private static void dmesg() {
        try {
            new ProcessBuilder("dmesg").redirectOutput(ConfigFileManager.getKmsgPath()).start();
        } catch (IOException e) {
            Log.e(TAG, "dmesg: ", e);
        }
    }
        ...
        try {
            ...
            if (stat.st_nlink == 0) {
                ...
                if (!Files.isDirectory(parent, LinkOption.NOFOLLOW_LINKS)) {
                    if (ConfigFileManager.chattr0(parent)) Files.deleteIfExists(parent);
                }

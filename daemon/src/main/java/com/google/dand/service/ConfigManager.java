...
import static com.google.dand.service.PackageService.MATCH_ALL_FLAGS;
import static com.google.dand.service.PackageService.PER_USER_RANGE;
import static com.google.dand.service.ServiceManager.TAG;
import static com.google.dand.service.ServiceManager.existsInGlobalNamespace;
import static com.google.dand.service.ServiceManager.toGlobalNamespace;
...
import com.google.dand.daemon.BuildConfig;
import com.google.dand.models.Application;
...
public class ConfigManager {
    ...
    private final SQLiteStatement createModulesTable = db.compileStatement("CREATE TABLE IF NOT EXISTS modules (" + "mid integer PRIMARY KEY AUTOINCREMENT," + "module_pkg_name text NOT NULL UNIQUE," + "apk_path text NOT NULL, " + "enabled BOOLEAN DEFAULT 0 " + "CHECK (enabled IN (0, 1))" + ");");
    private final SQLiteStatement createScopeTable = db.compileStatement("CREATE TABLE IF NOT EXISTS scope (" + "mid integer," + "app_pkg_name text NOT NULL," + "user_id integer NOT NULL," + "PRIMARY KEY (mid, app_pkg_name, user_id)," + "CONSTRAINT scope_module_constraint" + "  FOREIGN KEY (mid)" + "  REFERENCES modules (mid)" + "  ON DELETE CASCADE" + ");");
    private final SQLiteStatement createConfigTable = db.compileStatement("CREATE TABLE IF NOT EXISTS configs (" + "module_pkg_name text NOT NULL," + "user_id integer NOT NULL," + "`group` text NOT NULL," + "`key` text NOT NULL," + "data blob NOT NULL," + "PRIMARY KEY (module_pkg_name, user_id, `group`, `key`)," + "CONSTRAINT config_module_constraint" + "  FOREIGN KEY (module_pkg_name)" + "  REFERENCES modules (module_pkg_name)" + "  ON DELETE CASCADE" + ");");
    ...
    private static SQLiteDatabase openDb() {
        var params = new SQLiteDatabase.OpenParams.Builder().addOpenFlags(SQLiteDatabase.CREATE_IF_NECESSARY | SQLiteDatabase.ENABLE_WRITE_AHEAD_LOGGING).setErrorHandler(sqLiteDatabase -> Log.w(TAG, "database corrupted"));
    ...
    private synchronized void updateConfig() {
        Map<String, Object> config = getModulePrefs("dand", 0, "config");
        ...
        if (bool != null) {
            // TODO: remove
            updateModulePrefs("dand", 0, "config", "enable_auto_add_shortcut", null);
        }
        ...
        if (string == null) {
            ...
            updateModulePrefs("dand", 0, "config", "misc_path", miscPath.toString());
        } else {
    ...
    public synchronized void updateManager(boolean uninstalled) {
        ...
        try {
            ...
            Log.e(TAG, "failed to get manager info", ignored);
        }
    }
            ...
            switch (db.getVersion()) {
                case 0:
                    executeInTransaction(() -> {
                        ...
                        values.put("module_pkg_name", "dand");
    ...
    private @NonNull Map<String, HashMap<String, Object>> fetchModuleConfig(String name, int user_id) {
        ...
        try (Cursor cursor = db.query("configs", new String[]{"`group`", "`key`", "data"}, "module_pkg_name = ? and user_id = ?", new String[]{name, String.valueOf(user_id)}, null, null, null)) {
            if (cursor == null) {
    ...
    private synchronized void cacheModules() {
        ...
        try (Cursor cursor = db.query(true, "modules", new String[]{"module_pkg_name", "apk_path"}, "enabled = 1", null, null, null, null, null)) {
            if (cursor == null) {
                Log.e(TAG, "db cache failed");
                return;
            }
            ...
            while (cursor.moveToNext()) {
                ...
                if (packageName.equals("dand")) continue;
                var module = new Module();
            ...
            modules.stream().parallel().filter(m -> {
                ...
                if (oldModule != null && pkgInfo.applicationInfo.sourceDir != null && m.apkPath != null && oldModule.apkPath != null && existsInGlobalNamespace(m.apkPath) && Objects.equals(m.apkPath, oldModule.apkPath) && Objects.equals(new File(pkgInfo.applicationInfo.sourceDir).getParent(), new File(m.apkPath).getParent())) {
    ...
    private synchronized void cacheScopes() {
        ...
        cachedScope.clear();
        try (Cursor cursor = db.query("scope INNER JOIN modules ON scope.mid = modules.mid", new String[]{"app_pkg_name", "module_pkg_name", "user_id"}, "enabled = 1", null, null, null, null)) {
                    ...
                    for (ProcessScope processScope : processesScope) {
                        ...
                        if (modulePackageName.equals(app.packageName)) {
                            ...
                            for (var user : UserService.getUsers()) {
                                ...
                                cachedScope.computeIfAbsent(moduleSelf, ignored -> new LinkedList<>()).add(module);
                            }
                        }
                    }
                } catch (RemoteException e) {
    ...
    public List<Application> getModuleScope(String packageName) {
        if (packageName.equals("dand")) return null;
        try (Cursor cursor = db.query("scope INNER JOIN modules ON scope.mid = modules.mid", new String[]{"app_pkg_name", "user_id"}, "modules.module_pkg_name = ?", new String[]{packageName}, null, null, null)) {
            if (cursor == null) {
                return null;
            }
            int userIdIdx = cursor.getColumnIndex("user_id");
    ...
    public String getModuleApkPath(ApplicationInfo info) {
        ...
        var apkPath = Arrays.stream(apks).parallel().filter(apk -> {
            ...
            try (var zip = new ZipFile(toGlobalNamespace(apk))) {
                return zip.getEntry("META-INF/dandroid/java_init.list") != null || zip.getEntry("assets/dandroid_init") != null;
            } catch (IOException e) {
                return false;
            }
        }).findFirst();
    ...
    public boolean updateModuleApkPath(String packageName, String apkPath, boolean force) {
        if (apkPath == null || packageName.equals("dand")) return false;
        if (db.inTransaction()) {
    ...
    private int getModuleId(String packageName) {
        if (packageName.equals("dand")) return -1;
        if (db.inTransaction()) {
    ...
    public boolean setModuleScope(String packageName, String scopePackageName, int userId) {
        ...
        // Called by dandroid service, should be async
        updateCaches(false);
        return true;
    }
    ...
    public String[] enabledModules() {
        try (Cursor cursor = db.query("modules", new String[]{"module_pkg_name"}, "enabled = 1", null, null, null, null)) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (pkgName.equals("dand")) continue;
                result.add(pkgName);
            }
    ...
    private boolean removeModuleWithoutCache(String packageName) {
        if (packageName.equals("dand")) return false;
    ...
    private boolean removeModuleScopeWithoutCache(Application module) {
        if (module.packageName.equals("dand")) return false;
    ...
    private boolean removeAppWithoutCache(Application app) {
        return executeInTransaction(() -> db.delete("scope", "app_pkg_name = ? AND user_id=?", new String[]{app.packageName, String.valueOf(app.userId)}) > 0);
    }
    ...
    public void setVerboseLog(boolean on) {
        ...
        updateModulePrefs("dand", 0, "config", "enable_verbose_log", on);
        verboseLog = on;
    }
    ...
    public void setDexObfuscate(boolean on) {
        updateModulePrefs("dand", 0, "config", "enable_dex_obfuscate", on);
    }
    ...
    public void blockScopeRequest(String packageName) {
        ...
        set.add(packageName);
        updateModulePrefs("dand", 0, "config", "scope_request_blocked", set);
    ...
    boolean dexObfuscate() {
        var bool = getModulePrefs("dand", 0, "config").get("enable_dex_obfuscate");
    ...
    public void setEnableStatusNotification(boolean enable) {
        updateModulePrefs("dand", 0, "config", "enable_status_notification", enable);

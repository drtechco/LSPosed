...
import static com.google.dand.service.LSPNotificationManager.SCOPE_CHANNEL_ID;
import static com.google.dand.service.LSPNotificationManager.UPDATED_CHANNEL_ID;
...
import static com.google.dand.service.ServiceManager.getExecutorService;
...
import com.google.libdandroid.service.IDAndroidScopeCallback;
...
public class DAndroidService extends IDAndroidService.Stub {
    ...
    private static boolean isModernModules(ApplicationInfo info) {
        ...
        for (var apk : apks) {
            try (var zip = new ZipFile(apk)) {
                if (zip.getEntry("META-INF/dandroid/java_init.list") != null) {
                    return true;
                }
    ...
    private void dispatchPackageChanged(Intent intent) {
        ...
        boolean isDAndroidModule = applicationInfo != null && ((applicationInfo.metaData != null && applicationInfo.metaData.containsKey("dandroidminversion")) || isModernModules(applicationInfo));
        ...
        switch (intentAction) {
            ...
            case Intent.ACTION_PACKAGE_ADDED, Intent.ACTION_PACKAGE_CHANGED -> {
                ...
                if (isDAndroidModule) {
                    // When installing a new DAndroid module, we update the apk path to mark it as a
                    // module to send a broadcast when modules that have not been activated are
                    // uninstalled.
                    // If cache not updated, assume it's not dandroid module
                    isDAndroidModule = ConfigManager.getInstance().updateModuleApkPath(moduleName, ConfigManager.getInstance().getModuleApkPath(applicationInfo), false);
                ...
                broadcastAndShowNotification(moduleName, userId, intent, isDAndroidModule);
            }
        ...
        Log.d(TAG, "Package changed: uid=" + uid + " userId=" + userId + " action=" + intentAction + " isDAndroidModule=" + isDAndroidModule + " isAllUsers=" + allUsers);
    ...
    private void broadcastAndShowNotification(String packageName, int userId, Intent intent, boolean isDAndroidModule) {
        ...
        intent.putExtra("isDAndroidModule", isDAndroidModule);
    ...
    private void dispatchModuleScope(Intent intent) {
        ...
        var iCallback = IDAndroidScopeCallback.Stub.asInterface(callback);
        try {
